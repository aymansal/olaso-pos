...
assert.match(appBuild, /versionCode [1-9]\d*/);
assert.match(appBuild, /versionName "\d+\.\d+(?:\.\d+)?"/);
...
assert.match(secureSessionPlugin, /fun replace\(call: PluginCall\)/);
assert.match(secureSessionPlugin, /editor\.commit\(\)/);
assert.doesNotMatch(secureSessionPlugin, /\.apply\(\)/);
