...
try {
  ...
  assert.equal(LOCAL_SCHEMA_VERSION, 25);
  assert.equal(schemaVersion(database), 25);
  assert.ok(database.prepare("SELECT 1 FROM pragma_table_info('products') WHERE name = 'product_code'").get());
